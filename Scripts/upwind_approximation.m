function [ Q, t ] = upwind_approximation( gridpoints, initial_values, discrete_flux, timestepsize, finaltime)
...
for n = 1:T-1
    ...
    for j = 2:k
        a = Q(n, j);
        Q(n+1, j) = Q(n, j) - a*timestepsize/(gridpoints(j)-gridpoints(j-1))*( discrete_flux(Q(n,j), Q(n,j+1), a) - discrete_flux(Q(n,j-1), Q(n,j), a));    %calculation of the scheme
    end
end
